...
#     CASE WHEN ar.month is NUll THEN tr.month ELSE ar.month END as month,
...
app_data_and_volumes as(
Select trade_data.day, Replace(app_data.appdata::text, '"', '') as "appData", sum(trade_data."trade_value") as "sum_usd_volume" from decoded_trade_call_data_and_hash app_data 
inner join trades_with_prices trade_data 
on app_data."call_tx_hash" = trade_data."tx_hash"
and app_data."sellAmount"::numeric = trade_data."token_sold"::numeric 
-- and app_data."buyAmount"::numeric = trade_data."buyAmount"::numeric. /// We could do more checks in the future, tbd.
...
app_data_and_nr_of_referrals as(
Select  trade_data.day, Replace(app_data.appdata::text, '"', '') as "appData", count(distinct(trade_data.owner)) as "nr_of_referrals" from decoded_trade_call_data_and_hash app_data 
inner join trades_with_prices trade_data 
on app_data."call_tx_hash" = trade_data."tx_hash"
and app_data."sellAmount"::numeric = trade_data."token_sold"::numeric 
-- and app_data."buyAmount"::numeric = trade_data."buyAmount"::numeric. /// We could do more checks in the future, tbd.
...
trading_volume_results as ( 
    Select 
        Replace( Case When user_stats_general.owner is Null Then user_stats_of_gp.owner::TEXT ELSE user_stats_general.owner::TEXT END, '\', '0') as owner,
        Case When user_stats_general.day is NUll Then user_stats_of_gp.day ELSE user_stats_general.day END as day,
        number_of_trades, 
        cowswap_usd_volume,
        usd_volume_all_exchanges
    from user_stats_of_gp
    full outer join user_stats_general 
    on (user_stats_general.owner = user_stats_of_gp.owner and user_stats_general.day = user_stats_of_gp.day)
)
